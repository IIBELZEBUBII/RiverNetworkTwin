...
* Дополнительно: построить **подземные водоразделы и защитные зоны** на основе новых алгоритмов (см. раздел «Новые функции»).
...
- **Подземные водоразделы** (`src/underground/`): объединение DEM, поверхности грунтовых вод, проницаемости и карстового индекса в единый «стоимостной» растр, поиск LCP между источниками и выходами, автоматическое построение подземных полигонов.
- **Защитные зоны** (`src/protection/`): построение комбинированного буфера (поверхностный + подземный) вокруг полученных водоразделов и речной сети для оценки охранных территорий.
- **Риск эрозии почв** (`src/erosion/`): реализация факторов RUSLE (R, K, LS, C, P), расчёт слоя потерь почв и автоматическое выделение зон высокого риска.
- **Зоны выветривания** (`src/weathering/`): построение индекса выветривания на основе уклонов, влажности, литологии и температурных амплитуд, пересечение с водоразделами для выявления проблемных участков.
- **QGIS Processing-провайдер** (`src/processing/`): теперь включает пять алгоритмов (`build_underground_cost`, `underground_paths`, `protection_zones`, `soil_erosion`, `weathering_zones`), которые можно использовать в Processing Modeler и Python-консолях.
- В интерфейсе плагина появились две новые кнопки: «Подземные водоразделы» и «Защитные зоны».
