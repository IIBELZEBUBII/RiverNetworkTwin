...
from .erosion import soil_erosion_analysis
from .forest import forest
from .least_cost_path.least_cost_path import least_cost_path_analysis
from .processing import RiverNetworkProvider
from .protection import protection_zone_analysis
from .river.river import river
from .underground import underground_river_analysis
from .weathering import weathering_zone_analysis
...
class CustomDEMPlugin:
    def __init__(self, iface) -> None:
        ...
        self.processing_provider: Optional[RiverNetworkProvider] = None
    ...
    def initGui(self) -> None:
        ...
        if self.processing_provider is None:
            self.processing_provider = RiverNetworkProvider()
    ...
    def show_choice_dialog(self) -> None:
        ...
        underground_button = QPushButton("Подземные водоразделы")
        protection_button = QPushButton("Защитные зоны")
        erosion_button = QPushButton("Риск эрозии почв")
        weathering_button = QPushButton("Зоны выветривания")
        ...
        layout.addWidget(underground_button)
        layout.addWidget(protection_button)
        layout.addWidget(erosion_button)
        layout.addWidget(weathering_button)
        ...
        def create_underground() -> None:
            dialog.close()
            underground_river_analysis(self.project_folder)
            self.add_custom_path_button()

        def create_protection() -> None:
            dialog.close()
            protection_zone_analysis(self.project_folder)
            self.add_custom_path_button()

        def create_erosion() -> None:
            dialog.close()
            soil_erosion_analysis(self.project_folder)
            self.add_custom_path_button()

        def create_weathering() -> None:
            dialog.close()
            weathering_zone_analysis(self.project_folder)
            self.add_custom_path_button()
        ...
        underground_button.clicked.connect(create_underground)
        protection_button.clicked.connect(create_protection)
        erosion_button.clicked.connect(create_erosion)
        weathering_button.clicked.connect(create_weathering)
